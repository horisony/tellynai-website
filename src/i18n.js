...
export function useLanguage(){ const [language,setState]=useState(()=>localStorage.getItem('tellyn-language')||''); useEffect(()=>{const sync=(e)=>setState(e.detail);window.addEventListener('tellyn-language',sync);if(!language)(async()=>{try{let r=await fetch('/api/locale');if(!r.ok)r=await fetch('https://ipapi.co/json/');const d=await r.json();const next=fromCountry(d.country||d.country_code);setState(next);window.dispatchEvent(new CustomEvent('tellyn-language',{detail:next}))}catch{const b=navigator.language.toLowerCase();const next=b.startsWith('zh')?'zh':b.startsWith('ar')?'ar':'en';setState(next);window.dispatchEvent(new CustomEvent('tellyn-language',{detail:next}))}})();return()=>window.removeEventListener('tellyn-language',sync)},[]); return [language||'zh',(next)=>{localStorage.setItem('tellyn-language',next);setState(next);window.dispatchEvent(new CustomEvent('tellyn-language',{detail:next}))}] }
export function usePageTranslation(language){useLayoutEffect(()=>{document.documentElement.lang=language==='zh'?'zh-CN':language;document.documentElement.dir=language==='ar'?'rtl':'ltr'},[language])}
